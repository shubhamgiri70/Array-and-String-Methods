...
let averageGradeOfMale =
  ...
  }, 0) / persons.filter((person) => person.sex === "M").length;
...
let averageGradeOfFemale =
  ...
  }, 0) / persons.filter((person) => person.sex === "F").length;
...
let highestGradePeople = persons
  .filter(
...
const fruitsObj = fruitBasket.reduce((object, fruit) => {
  if (object[fruit]) {
    object[fruit]++;
  } else {
    object[fruit] = 1;
...
const fruitArr = Object.keys(fruitsObj).map((fruit) => [
  fruit,
  fruitsObj[fruit],
]);
...
const flatData = data.reduce((acc, cv) => {
...
const flatDataTwo = dataTwo.reduce((acc, cv) => {
